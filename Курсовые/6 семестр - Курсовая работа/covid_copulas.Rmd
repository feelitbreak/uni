...
my_precovid_data = my_data[my_data$`Дата` < "2020-01-31", ]
my_covid_data = my_data[my_data$`Дата` >= "2020-01-31", ]
...
for (i in 1:(n - 1)) {
  for (j in i:(n - 1)) {
    BiCopKPlot(pseudo_precovid_returns[, i], pseudo_precovid_returns[, j])
  }
}
```
...
for (i in 1:(n - 2)) {
  for (j in i:(n - 2)) {
    ...
    plot(as.numeric(pseudo_res_precovid[, i]), as.numeric(pseudo_res_precovid[, j]), main = sprintf('Test dataset x and y, i = %d, j = %d', i, j), col = "blue")
...
for (i in 1:(n - 2)) {
  for (j in i:(n - 2)) {
    ...
    plot(as.numeric(pseudo_res_covid[, i]), as.numeric(pseudo_res_covid[, j]), main = sprintf('Test dataset x and y, i = %d, j = %d', i, j), col = "blue")
    points(simdata[,1], simdata[,2], col = 'red')
    legend('bottomright', c('Наблюдаемые', 'Сгенерированные'), col = c('blue', 'red'), pch=21)
  }
}
