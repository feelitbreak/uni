...
public class Lab {
    ...
    if (bigX.compareTo(minInterval) != 1 || bigX.compareTo(maxInterval) != -1) {
    	out.println("Ошибка. x должен принадлежать интервалу (-1; 1).");
    	return;
    }
    ...
    out.println("Значение выражения через BigDecimal  = " + y);
    out.println("Значение выражения через Math  = " + 1/Math.sqrt(x+1));
